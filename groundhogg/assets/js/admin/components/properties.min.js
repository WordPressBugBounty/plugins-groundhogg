($=>{const{uuid,specialChars,icons,modal,copyObject,input,textarea,select,tinymceElement,moreMenu,tooltip,inputRepeaterWidget,confirmationModal,dangerConfirmationModal,toggle}=Groundhogg.element;const{sprintf,__,_x,_n}=wp.i18n;const optionsRepeater=({selector,options,onChange})=>{inputRepeaterWidget({selector:selector,rows:options.map(o=>[o]),cellCallbacks:[input],cellProps:[{placeholder:__("Option")}],sortable:true,onChange:r=>{onChange(r.map(r=>r[0]))}}).mount()};const standardField={onMount:({id,name},onChange)=>{$(`#${id}`).on("change",e=>{onChange({[name]:e.target.value})})},edit:()=>{return""},onEditMount:()=>{}};const getFieldType=type=>{return{...standardField,...fieldTypes[type]}};const fieldTypes={text:{name:__("Text","groundhogg"),view:({label,...props})=>{return`<label class="property-label" for="${props.id}">${label}</label>${input({className:"full-width",...props,type:"text"})}`}},textarea:{name:__("Textarea","groundhogg"),view:({label,...props})=>{return`<label class="property-label" for="${props.id}">${label}</label>${textarea({className:"full-width",...props})}`}},number:{name:__("Number","groundhogg"),view:({label,decimals=0,...props})=>{let step=(1/Math.pow(10,decimals)).toString();return`<label class="property-label" for="${props.id}">${label}</label>${input({className:"full-width",...props,type:"number",step:step})}`},edit:field=>{const{decimals=0}=field;return`
			<div class="gh-rows-and-columns">
				<div class="gh-row">
					<div class="gh-col">
						<label>${__("Decimals","groundhogg")}</label>
						${input({type:"number",name:"property_number_decimals",id:"property-number-decimals",value:decimals,min:0,max:100})}
					</div>
				</div>
			</div>
        `},onEditMount:(field,updateField)=>{$("#property-number-decimals").on("change",e=>{updateField({decimals:e.target.value})})}},url:{name:__("URL","groundhogg"),view:({label,...props})=>{return`<label class="property-label" for="${props.id}">${label}</label>
		<div class="gh-input-group">${input({className:"full-width",...props,type:"url"})}
			<button id="link-${props.id}" class="gh-button secondary small icon">
				<span class="dashicons dashicons-external"></span>
			</button>
		</div>`},onMount:({id,name},onChange)=>{const $input=$(`#${id}`);$input.on("change",e=>{onChange({[name]:e.target.value})});$(`#link-${id}`).on("click",e=>{let link=$input.val();if(link){window.open(link,"_blank")}})}},tel:{name:__("Phone Number","groundhogg"),view:({label,...props})=>{return`<label class="property-label" for="${props.id}">${label}</label>${input({className:"full-width",...props,type:"tel"})}`}},custom_email:{name:__("Email Address","groundhogg"),view:({label,...props})=>{return`<label class="property-label" for="${props.id}">${label}</label>${input({className:"full-width",...props,type:"email"})}`}},date:{name:__("Date","groundhogg"),view:({label,...props})=>{return`<label class="property-label" for="${props.id}">${label}</label>${input({className:"full-width",...props,type:"date"})}`}},time:{name:__("Time","groundhogg"),view:({label,...props})=>{return`<label class="property-label" for="${props.id}">${label}</label>${input({className:"full-width",...props,type:"time"})}`}},datetime:{name:__("Date & Time","groundhogg"),view:({label,...props})=>{return`<label class="property-label" for="${props.id}">${label}</label>${input({className:"full-width",...props,type:"datetime-local"})}`}},checkboxes:{name:__("Checkboxes","groundhogg"),view:({label,id,name,options,value,...props})=>{if(!Array.isArray(value)){value=value.split(",").map(v=>v.trim())}return`<label class="property-label">${label}</label>
		${options.map(opt=>`<label class="checkbox-label">${input({type:"checkbox",value:opt,dataId:id,checked:value.includes(opt)})} ${opt}</label>`).join("")}`},onMount:({id,name},onChange)=>{$(`input[type=checkbox][data-id=${id}]`).on("change",e=>{let checked=[];$(`input[type=checkbox][data-id=${id}]`).each((i,e)=>{if(e.checked){checked.push(e.value)}});onChange({[name]:checked})})},edit:field=>{const{multiple,blankOption}=field;return`
			<div class="gh-rows-and-columns">
				<div class="gh-row">
					<div class="gh-col">
        ...
        `},onEditMount:(field,updateField)=>{optionsRepeater({selector:"#property-dropdown-options",options:field.options||[""],onChange:options=>updateField({options:options})})}},radio:{name:__("Radio Buttons","groundhogg"),view:({label,id,name,options,value,...props})=>{return`<label class="property-label">${label}</label>
		${options.map(opt=>`<label class="checkbox-label">${input({type:"radio",value:opt,name:name,checked:value===opt})} ${opt}</label>`).join("")}`},onMount:({id,name},onChange)=>{$(`input[type=radio][name=${name}]`).on("change",e=>{onChange({[name]:e.target.value})})},edit:field=>{return`
			<div class="gh-rows-and-columns">
				<div class="gh-row">
					<div class="gh-col">
						<label>${__("Options","groundhogg")}</label>
        ...
        `},onEditMount:(field,updateField)=>{optionsRepeater({selector:"#property-dropdown-options",options:field.options||[""],onChange:options=>updateField({options:options}),sortable:true});$("#allow-multiple").on("change",e=>{updateField({multiple:e.target.checked})})}},html:{name:__("HTML","groundhogg"),view:({label,...props})=>{return`<label class="property-label" for="${props.id}">${label}</label>${textarea({className:"full-width",...props})}`},onMount:({id,name,...props},onChange)=>{wp.editor.remove(id);let editor=tinymceElement(id,{quicktags:false},content=>{onChange({[name]:content})})}}};const Templates={noProperties:()=>{return`
		  <button id="add-custom-property" class="gh-button secondary">
			  ${__("Add custom properties","groundhogg")}
		  </button>`},groups:({groups,fields=[]},editable)=>{return`
		  <div class="property-groups">
			  ${groups.map(g=>Templates.group(g,fields.filter(f=>f.group==g.id),editable)).join("")}
		  </div>`},group:(group,fields=[],editable)=>{fields.sort(({order:a=10},{order:b=10})=>a-b);return`
		  ...
		  </div>`},renamePropertyGroup:name=>{return`
		  <div class="property-group">
		  ...
		  </div>`},addField:field=>{const{type,label,name,id,order=10,width=2}=field;let editUI="";try{editUI=getFieldType(type).edit(field)}catch(e){console.log(e)}return`
			  ...
			  <div class="gh-rows-and-columns">
				  ...
				  <div class="gh-row">
					  <div class="gh-col">
						  ...
						  ${select({id:"property-field-type",options:Object.keys(fieldTypes).map(type=>({value:type,text:fieldTypes[type].name})),selected:type})}
					  </div>
				  </div>
				  <div class="gh-row">
					  <div class="gh-col">
				  ...
				  <div class="gh-row">
					  <div class="gh-col">
						  ...
						  ${input({id:"property-field-order",name:"property_field_order",placeholder:__("10","groundhogg"),value:order,type:"number"})}
					  </div>
					  <div class="gh-col">
						  <label class="">${__("Width","groundhogg")}</label>
						  ${select({id:"property-field-width",name:"property_field_width"},{1:__("Full"),2:__("Half")},width)}
					  </div>
				  </div>
          <div class="gh-row">
              <div class="gh-col">
