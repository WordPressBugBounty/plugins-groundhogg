          ...
          showBack: true, inside: () => {
            ...
            return `
                ...
                ${ faq({
                    ...
                    //language=HTML
                    content: `<p>${ __(
                            'An email you are trying to send in a flow or broadcast is not marked as ready. All emails must be marked as ready before they can be sent.',
        ...
        return stepTemplate({
          showBack: true, inside: () => {
            ...
            return `
                ...
                <p>
                    ${ __('A cron job is a process which ensures that all your WordPress processes run on time. Like sending emails or processing flow events.') }</p>
                <p>
...
<p><a href="https://groundhogg.io/doc/cron-jobs/#cronjoborg" target="_blank">${ __(
    ...
    },
    {
      id: 'other-issue', render: () => {
                ...
                <ul>
                    ...
                    <li>
                        <button id="funnel-not-running"
                                class="gh-button text secondary">
                            ${ __('My flow is not running', 'groundhogg') }
                        </button>
                    </li>
                    <li>
                        <button id="funnel-not-working"
                                class="gh-button text secondary">
                            ${ __('My flow is not working the way it is supposed to', 'groundhogg') }
                        </button>
                    </li>
                    <li>
        ...
        $('#funnel-not-working').
          on('click', () => next('funnel-not-working') )
    ...
    },
    {
      ...
      render: () => {
        ...
        return stepTemplate({
          showBack: true, inside: () => {
            // language=HTML
            return `
                <h1>${ __('Issue with your flow?', 'groundhogg') }</h1>
                <p>
                    ${ __('Which flows are you having problems with?') }</p>
                <div class="inside">
                    ${ select({
                        id: 'funnels',
                    }) }
      ...
      onMount: ({ next }) => {
        ...
        $('#ticket').on('click', () => {
          ...
          if (!funnels.length) {
            dialog({
              message: __('Select a flow first!'),
              type: 'error',
            })
            return
          }

          ticket.subject = 'Issue with my flows'
          //language=HTML
          ticket.message = `
              <p><i>This message was generated by the Groundhogg troubleshooter.</i></p>
              <p>I'm having issues with the following flows:</p>
              <ul>
    ...
    },
    {
      ...
      onMount: ({ next, prev }) => {
        ...
        const setDocResults = (results) => {

          $('#doc-results').
            html(`${ results.map(a => `<li>📄 <a href="${ a.link }" target="_blank">${ a.title.rendered }</a></li>`).
              join('') }
                <div class="space-between align-center"
                     style="margin-top: 40px">
                    <button id="ticket" class="gh-button secondary text">
        ...
        const searchDocs = Groundhogg.functions.debounce( async () => {
          ...
          inside: () => {
            ...
            return `
                ...
                <ul>
                    ...
                    <li><a href="https://groundhogg.io/documentation/" target="_blank">${ __('Search our documentation.') }</a>
                    </li>
                    <li><a href="https://groundhogg.io/events/" target="_blank">${ __(
                            'Watch a training on Groundhogg.') }</a></li>
          ...
          inside: () => {
            ...
            return `
                ...
                <ul>
                    ...
                    <li><a href="https://groundhogg.io/events" target="_blank">${ __(
                            'Watch a training video on Groundhogg.') }</a></li>
                    <li><a href="https://www.youtube.com/Groundhogg" target="_blank">${ __(
                            'Subscribe to our YouTube channel!') }</a></li>
                </ul>
                <div class="space-between align-right gap-10"
